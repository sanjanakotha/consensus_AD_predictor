...
def wrapper_pred(seq, ss, dis_short, dis_long):
    ...
    # Load PADDLE model once per worker and store it on the worker
    if not hasattr(worker, "paddle_model"):
        from PADDLE import paddle
        worker.paddle_model = paddle.PADDLE()

        dummy_len = 53
        dummy_prot = "A" * dummy_len
        dummy_helix = np.zeros(dummy_len)
        dummy_coil = np.zeros(dummy_len)
        dummy_dis_short = np.zeros(dummy_len)
        dummy_dis_long = np.zeros(dummy_len)
        worker.paddle_model.predict_protein(dummy_prot, dummy_helix, dummy_coil, dummy_dis_short, dummy_dis_long)


    pad = worker.paddle_model
...
def main(fasta_name, output_dir):
    ...
    sec_structs = [wrapper_ss(r, output_dir) for r in recs]
    
    # This is where the parallelization happens
    futures = client.map(wrapper_pred, sequences, sec_structs, dis_shorts, dis_longs) 
